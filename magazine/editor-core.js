...
async function loadProfile() {
  ...
  try {
    ...
    profileCache = {
      ...
      pairs: data.pairs || [], // 2026-07-02: 매거진도 AI초안→vase수정본 pairs를 읽도록 연결
    };
  } catch (e) {
    profileCache = { rules: [...DEFAULT_RULES], examples: [], pairs: [] };
  }
  return profileCache;
}
...
${SapConstitution.buildConstitutionText()}

(제목/부제 생성 시에도 위 삽만리 문체 철학·금지 규칙을 그대로 따른다. 완성글을 한 번에 쓰지 말고 장면 → 사실 → 리듬 순으로 전개)
...
${SapConstitution.buildAvoidExamplesText()}
