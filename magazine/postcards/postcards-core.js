...
const POSTCARDS_FILE = 'magazine/postcards/postcards.json';
  ...
  try {
    for (const path of selectedPaths) {
      ...
      const pc = {
        ...
        image: '../images/library/' + item.name,
        imageHash: item.hash,
        label: '',
        title: '',
...
async function reAnalyze(idx) {
  ...
  const item = libraryItems.find(x => x.hash === pc.imageHash) || { hash: pc.imageHash, path: pc.image.replace('../', 'magazine/') };
