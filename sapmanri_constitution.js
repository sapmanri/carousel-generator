...
- 제목, 본문, SEO Focus keyphrase, Meta description을 포함한다`
  ...
  const SAPMANRI_CONSTITUTION = {
    ...
    observation: [
      ...
      // 2026-07-02 병합 (Style Rule Audit D①클러스터): profile_data.json에 9곳 중복되어 있던
      // "예쁘게 묘사·상징화 금지" 계열 철학 규칙을 여기로 승격. 원문은 Archive 보존.
      // (D②③ — 톤 가이드, 의인화 금지 등 구체 체크리스트는 2차 작업에서 profile_data.json에 남김)
      '사진 속 사물을 상징으로 키우지 않는다. 보이는 것 1~2개만 고르고, 그 앞에서 내가 어떻게 늦어졌는지로 빠르게 넘어간다.'
    ],
    thinking: [
    ...
    sentence: [
      ...
      // 2026-07-02 병합 (Style Rule Audit C클러스터): profile_data.json에 동일 취지 규칙이
      // 7곳에 중복되어 있던 것을 여기로 승격, 원문은 Decision Log/Archive에 보존.
      '마지막 문장은 정해진 결말 문구로 닫지 않는다. 의미를 요약하지 말고, 장면 안에 남아 있는 사물·소리·빛·행동 중 하나로 닫는다.',
    ...
    avoid: [
      ...
      // 2026-07-02 병합 (Style Rule Audit B클러스터): profile_data.json에 7곳 중복되어 있던
      // "감정을 직접 설명하지 말라" 계열 규칙을 여기로 승격. 원문은 Archive 보존.
      '감정을 직접 설명하지 않는다. 사물과 장면, 몸의 구체적 행동으로 감정이 드러나게 한다.'
    ],
    // 2026-07-02 Style Rule Audit 2차: 삭제하지 않고 '프롬프트 제외 후보'로만 표시.
    // 실행 지침이 아니라 관찰 일지 성격이라 buildConstitutionText()에서 빼는 게 맞다고 판단됐으나,
    // 아직 코드 동작은 바꾸지 않음 (다음 라운드에서 실제 제외 여부 결정).
    evolution: [
      '최근 문체는 감정보다 인지에 가깝다.',
